...
static struct gapm_start_advertise_cmd* app_easy_gap_undirected_advertise_start_create_msg(void)
{
    ...
    if (adv_cmd == NULL)
    {
        ...
        if (strlen(device_config) > 0)
        {
            ...
            uint16_t total_adv_space = 3 + adv_cmd->info.host.adv_data_len + 2 + strlen(device_config);
            // Get remaining space in the Scan Response Data ( plus 2 bytes are used for the length and flag bytes of the Device Name)
            uint16_t total_scan_space = adv_cmd->info.host.scan_rsp_data_len + 2 + strlen(device_config);
...
void app_init(void)
{
    ...
    device_info.dev_name.length = (strlen(device_config) <= GAP_MAX_NAME_SIZE) ? strlen(device_config) : GAP_MAX_NAME_SIZE;
    memcpy(device_info.dev_name.name, device_config, device_info.dev_name.length);
