...
void i2c_read(uint8_t reg, uint8_t * bufp, uint16_t len) {
  i2c_abort_t abort_code;
	//uint16_t length= 2;
  i2c_master_transmit_buffer_sync( &reg, len, &abort_code, I2C_F_NONE);
 i2c_master_receive_buffer_sync(bufp, len, &abort_code, I2C_F_ADD_STOP);
}
void i2c_write(uint8_t reg,const uint8_t data, uint16_t leng) {
  i2c_abort_t abort_code;

  // Transmit the register address first

  i2c_master_transmit_buffer_sync( &reg, leng, &abort_code, I2C_F_NONE);

  // Then transmit the data
 i2c_master_transmit_buffer_sync(&data, leng, &abort_code, I2C_F_NONE);
}
void i2c_bme_initialize(void)
{
    i2c_init(&i2c_cfg);
}